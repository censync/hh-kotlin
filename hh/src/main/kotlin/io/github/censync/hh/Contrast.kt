...
internal object Contrast {
    ...
    fun luminance(rgb: Int): Long =
        2126L * LIN[(rgb ushr 16) and 0xFF] + 7152L * LIN[(rgb ushr 8) and 0xFF] + 722L * LIN[rgb and 0xFF]

    /** The contrast ratio times 100, rounded down. */
    fun ratioX100(a: Int, b: Int): Int {
        val ya = luminance(a)
        val yb = luminance(b)
        val hi = maxOf(ya, yb) + 500000000L
        val lo = minOf(ya, yb) + 500000000L
        return (100L * hi / lo).toInt()
    }

    /** [rgb] with alpha [alpha] laid over [under]. */
    fun over(rgb: Int, alpha: Int, under: Int): Int {
        var out = 0
        for (shift in intArrayOf(16, 8, 0)) {
            val top = (rgb ushr shift) and 0xFF
            val bottom = (under ushr shift) and 0xFF
            out = out or (((alpha * top + (255 - alpha) * bottom + 127) / 255) shl shift)
        }
        return out
    }

    /** The weakest palette colour against [background]. */
    fun figuresX100(background: Int): Int = Palette.COLOURS.minOf { ratioX100(it, background) }
}
